...
- Generate demo emails (.eml files):
  - `python tools/email_generator.py --out-dir data/demo_inbox --count 20`
  - Creates `.eml` test messages and `email_index.csv` for reference.
- Ingest emails into the Excel-backed queue:
  - Folder mode: `python tools/email_ingest.py --folder data/demo_inbox --queue data/email_queue.xlsx --watch --poll-interval 10`
